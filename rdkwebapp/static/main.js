...
document.addEventListener('DOMContentLoaded', function() {
        ...
        if (confirm('确定要重置所有统计数据吗？')) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    charts.peopleChart.data.labels = [];
                    charts.peopleChart.data.datasets[0].data = [];
                    charts.peopleChart.update();
                    
                    if (charts.distributionChart) {
                        updateDistributionChart(charts.distributionChart, []);
                    }
                }
            });
        }
    ...
    document.getElementById('startBtn').addEventListener('click', function() {
        ...
        if (icon.classList.contains('fa-play')) {
            ...
            // 恢复数据更新
            updateInterval = setInterval(updateAllData, 2000);
        } else {
